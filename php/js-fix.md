...
## フォーカス制御（utils/focusControl.js）
- モーダル・ドロワー等のオーバーレイで使う「フォーカス移動・トラップ・復帰」は **utils/focusControl.js** に共通化する
- 他モジュールでも同じ挙動が必要な場合はこの util を利用する（柔軟にオプションで指定可能）
- 主な API：`getFocusables`（コンテナ内のフォーカス可能要素取得）、`focusFirst`（先頭へフォーカス・fallback 指定可）、`handleFocusTrapKeydown`（Tab でコンテナ内に留める）、`returnFocus`（閉じたときにトリガーへ戻す）、`prepareAndFocusContainer`（表示直後にコンテナ内へフォーカス）
